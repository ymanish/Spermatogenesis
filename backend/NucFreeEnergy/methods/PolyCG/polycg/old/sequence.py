...
def unique_oli_seq(
    ...
    for c in boundary + exclude:
        chars = all_chars.replace(c, "")
    # necessary number of chars
    required_chars = int(np.ceil(np.log(N) / np.log(oli_size)))
    chars = all_chars[:required_chars]
    # calculate sequence
    seq = unique_seq_of_chars(oli_size, chars, N=N)
    # match terminal seq to impose closure
    if closed:
        next_char = all_chars[required_chars]
        seq = seq[:-1] + next_char
        chars += next_char
    return seq, chars
