...
def eval_datadicts(params,args):
    ...
    diags = np.concatenate([params[symtriads.dof:symtriads.dof+6] for i in range(len(mu0))])
    K = np.diag(diags)
...
def eval_sqdiff(params,args):
    ...
    for datadict in datadicts:
        ...
        # plot_helicoidal(helicoidal_pdb,helicoidal_theory)
...
def fit_binding_model(
    ...
    res = sp.optimize.minimize(helicoidal_residue, params, method='Powell',args=(args,),options={'maxiter':10000000},tol=tol)
    
    fitparams = res.x
    
    mu0_deform = fitparams[:symtriads.dof].reshape(symtriads.deform_shape)
    mu0 = symtriads.deform2mu0(mu0_deform)
    K1diag = fitparams[symtriads.dof:symtriads.dof+6]
