...
from .output import merge_markov_output_files


def _reservoir_sample(iterable, sample_size: int, seed: int) -> list:
...
def run_markov_solver(
    ...
    Args:
        ...
        subids_range:   Tuple (start, end) for subid filtering (old format only)
    """
    if logger is None:
    ...
    if dataset_dir is not None:
        logger.info(f"Input (SPRM): {dataset_dir}")
    else:
        logger.info(f"Input (old format): {file_path}")
    ...
    logger.info(f"Eads delta: {config.eads_delta} kT")
    logger.info(f"Eads weight mode: {config.eads_weight_mode}")
    logger.info(f"Eads apply: {config.eads_apply}")
    if config.tnp2.enabled:
        logger.info(
            f"TNP2 J_eff ENABLED: eps_cpg={config.tnp2.eps_cpg}, "
            f"mu_t0={config.tnp2.mu_t0}"
        )
        if fasta_path is None:
            raise ValueError(
                "TNP2 layer is enabled but fasta_path was not provided. "
                "Pass fasta_path so sequences can be attached to nucleosomes; "
                "otherwise the layer falls back to F=1.0 silently."
            )
        logger.info(f"FASTA: {fasta_path} (id_style={fasta_id_style})")
    elif fasta_path is not None:
        logger.info(f"FASTA: {fasta_path} (id_style={fasta_id_style}) - attached but TNP2 disabled")
    logger.info(f"Time grid: {config.tau_steps} points, τ_max={config.tau_max}")
    logger.info(f"Method: {config.method}")
    logger.info(f"Workers: {config.n_workers}")
    logger.info(f"Batch size: {config.batch_size}")
    if (max_nucs if max_nucs is not None else config.max_nucs) is not None:
        logger.info(f"Random max_nucs seed: {config.max_nucs_seed}")
    ...
    if dataset_dir is not None:
        ...
        ERROR_MSG = (
            "Using old-format TSV input with nucleosome_generator. "
            "This is not recommended for new analyses. "
            "Consider converting to SPRM format and using nucleosome_generator_sprm for better performance and features."
        )
    ...
    if max_nucs is not None:
        gen = _reservoir_sample(gen, max_nucs, config.max_nucs_seed)
        logger.info(f"Randomly sampled {len(gen)} nucleosomes with seed {config.max_nucs_seed}")
