...
class DriftReversalResults:
    ...
    # Landscape descriptors (sequence-intrinsic, protamine-independent)
    dE_firstbreath: float  # min first-step opening cost relative to ground state
    dE_barrier: float      # max F_nuc(l,r) - F_nuc(0,0) over all transient (l,r)
    sigma_dE: float        # std of all single-step opening ΔE values (ruggedness)
...
class DriftReversalAnalyzer:
    ...
    def compute_landscape_descriptors(self) -> Dict[str, float]:
        """
        Compute three sequence-intrinsic energy-landscape descriptors.
        Protamine parameters are not used — purely G_mat observables.

        Returns
        -------
        dict with keys:
            dE_firstbreath : float
                min(F_nuc(1,0), F_nuc(0,1)) - F_nuc(0,0)
                Cheapest first opening step from the ground state.
                Negative means the first breath is energetically downhill.
            dE_barrier : float
                max over all transient (l,r) with l+r > 0 of [F_nuc(l,r) - F_nuc(0,0)]
                Highest point in the full 2D landscape relative to ground state.
            sigma_dE : float
                std of every single-step opening ΔF across the landscape.
        ...
        # ΔE_barrier and σ_ΔE — iterate over all transient states
        barrier_vals: List[float] = []
        opening_dEs: List[float] = []
        for l in range(N):
            for r in range(N - l):
                F_curr = self.F_nuc(l, r)
                if l + r > 0:
                    barrier_vals.append(F_curr - F0)
                for dl, dr in [(1, 0), (0, 1)]:
                    ln, rn = l + dl, r + dr
                    F_new = 0.0 if ln + rn >= N else self.F_nuc(ln, rn)
                    opening_dEs.append(F_new - F_curr)

        dE_barrier = float(max(barrier_vals)) if barrier_vals else 0.0
        sigma_dE   = float(np.std(opening_dEs)) if opening_dEs else 0.0

        return {
            'dE_firstbreath': dE_firstbreath,
    ...
    def analyze(self) -> DriftReversalResults:
        ...
        # 9. Landscape descriptors
        ld = self.compute_landscape_descriptors()
        ...
        results = DriftReversalResults(
            ...
            dE_firstbreath=ld['dE_firstbreath'],
            dE_barrier=ld['dE_barrier'],
            sigma_dE=ld['sigma_dE'],
            k_wrap=self.k_wrap,
